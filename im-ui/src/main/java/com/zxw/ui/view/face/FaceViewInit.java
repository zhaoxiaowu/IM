 ...
 */
public class FaceViewInit {

    private FaceView faceView;

    public FaceViewInit(FaceView faceView) {
    ...
    private void initFaces() {
        ...
        Pane rootPane = faceView.rootPane;
        ...
        for (Node next : children) {
            next.setOnMouseClicked(event -> {
                ...
                MultipleSelectionModel selectionModel = faceView.chatView.$("talkList", ListView.class).getSelectionModel();
                Pane selectedItem = (Pane) selectionModel.getSelectedItem();
                // 对话信息
                TalkBoxData talkBoxData = (TalkBoxData) selectedItem.getUserData();
                Date msgDate = new Date();
                String faceId = (String) next.getUserData();
                faceView.chatMethod.addTalkMsgRight(talkBoxData.getTalkId(), faceId, 1, msgDate, true, true, false);
                // 发送消息
                faceView.chatEvent.doSendMsg(faceView.chatView.userId, talkBoxData.getTalkId(), talkBoxData.getTalkType(), faceId, 1, msgDate);
